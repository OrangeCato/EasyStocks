...
//   function tone({ freq = 880, durationMs = 140, type = "sine", gain = 0.09 }) {
//     const c = ensureCtx();
//     const o = c.createOscillator();
//     const g = c.createGain();
...
//     // Tiny ramp in/out to avoid clicks
//     const now = c.currentTime;
//     g.gain.setValueAtTime(0.0001, now);
//     g.gain.exponentialRampToValueAtTime(gain, now + 0.01);
//     g.gain.exponentialRampToValueAtTime(0.0001, now + durationMs / 1000);
...
//   // More “pleasant” patterns, not arcade
//   function playAlertSound(alert) {
//     const isUp = alert?.op === ">";
//     const isDown = alert?.op === "<";
//     const isPrice = alert?.type === "price";
...
//     // PRICE ABOVE: upward “tick-tick”
//     if (isPrice && isUp) {
//       tone({ freq: 740, durationMs: 110, type: "triangle", gain: 0.10 });
//       setTimeout(() => tone({ freq: 920, durationMs: 130, type: "triangle", gain: 0.10 }), 120);
//       return;
//     }

//     // PRICE BELOW: downward “tick-tick”
//     if (isPrice && isDown) {
//       tone({ freq: 680, durationMs: 110, type: "triangle", gain: 0.10 });
//       setTimeout(() => tone({ freq: 520, durationMs: 130, type: "triangle", gain: 0.10 }), 120);
//       return;
//     }

//     // % UP: soft “ping”
//     if (isPct && isUp) {
//       tone({ freq: 980, durationMs: 150, type: "sine", gain: 0.085 });
//       setTimeout(() => tone({ freq: 1220, durationMs: 120, type: "sine", gain: 0.075 }), 120);
//       return;
//     }

//     // % DOWN: soft “pong”
//     if (isPct && isDown) {
//       tone({ freq: 820, durationMs: 150, type: "sine", gain: 0.085 });
//       setTimeout(() => tone({ freq: 640, durationMs: 120, type: "sine", gain: 0.075 }), 120);
//       return;
//     }

//     tone({ freq: 880, durationMs: 160, type: "sine", gain: 0.085 });
//   }
...
//   // avoid replaying same alerts forever
//   const lastPlayedAtRef = useRef(new Map()); // id -> timestamp ms
//   const COOLDOWN_MS = 2 * 60 * 1000; // 2 minutes per alert id

//   const audioRef = useRef(null);
//   if (!audioRef.current) audioRef.current = makeBeepPlayer();

//   // ---- Load listings (Top Rankings) ----
//   useEffect(() => {
//     let cancelled = false;
...
//   // ---- Load triggered alerts + sound for newly triggered ----
//   useEffect(() => {
//     let cancelled = false;
...
//         if (soundOn && triggered.length > 0) {
//           for (const a of triggered) {
//             const id = a.id || `${a.symbol}-${a.type}-${a.op}-${a.value}`;
//             const last = lastPlayedAtRef.current.get(id) || 0;

//             // only play if not played recently
//             if (now - last > COOLDOWN_MS) {
//               audioRef.current.playAlertSound(a);
//               lastPlayedAtRef.current.set(id, now);
//             }
//           }
//         }
...
//       className="es-btn"
//       onClick={() => {
//         // This click is required to unlock audio in most browsers
//         try {
//           audioRef.current.ensureCtx();
//         } catch {}
//         setSoundOn((v) => !v);
